...
class Renderer:
    ...
    async def __sync_modes(self, fsm_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.bot_modes is not None:
            if "__modes__" in fsm_data.keys():
                # Если число режимов изменилось, обновляем список режимов в fsm
                if len(self.bot_modes.dict_values) != len(fsm_data["__modes__"]):
                    fsm_data["__modes__"] = self.bot_modes.dict_values

                # Дополнительно проверяем ключи и значения режимов на изменения
                for name, values in fsm_data["__modes__"].items():
                    if name not in self.bot_modes.dict_values.keys():
                        fsm_data["__modes__"] = self.bot_modes.dict_values
                        break
                    for value in self.bot_modes.dict_values[name]:
            ...
            # Если режимы в боте не указаны мы убираем их из fsm
            if "__modes__" in fsm_data:
    ...
    async def render(self, window: str | Alert, chat_id: int, data: dict[str, Any] = None, message_id: int = None,
        ...
        if isinstance(window, Alert):
            ...
            await self.fsm.set_state(state=window)
        ...
        text, reply_markup = await window.assemble(data=window_data, modes=fsm_data["__modes__"])
    ...
    async def answer(self, window: str | Alert, chat_id: int, data: dict[str, Any] = None,
    ...
    async def edit(self, window: str | Alert, chat_id: int, message_id: int, data: dict[str, Any] = None,
                   parse_mode: ParseMode = Default("parse_mode")) -> tuple[Message, Window]:
    ...
    async def delete_and_send(self, window: str | Alert, chat_id: int, message_id: int, data: dict[str, Any] = None,
                              parse_mode: ParseMode = Default("parse_mode")) -> tuple[Message, Window]:
        return await self.render(window=window, chat_id=chat_id, message_id=message_id, parse_mode=parse_mode,
                                 mode=RenderMode.DELETE_AND_SEND, data=data)

    async def reply(self, window: str | Alert, chat_id: int, message_id: int, data: dict[str, Any] = None,
                    parse_mode: ParseMode = Default("parse_mode")) -> tuple[Message, Window]:
        return await self.render(window=window, chat_id=chat_id, message_id=message_id, parse_mode=parse_mode,
